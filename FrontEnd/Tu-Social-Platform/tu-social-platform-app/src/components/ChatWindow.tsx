...
//   {!isGroup && otherAvatarUrl ? (
//     <img src={otherAvatarUrl} alt={chatTitle} />
//   ) : (
...
//   {m.imageUrl ? (
//     <img src={m.imageUrl} alt={fullName} />
//   ) : (
...
import { chatSocket } from "../services/ChatSocket"; // adjust if your file name differs
...
export const ChatWindow: React.FC<ChatWindowProps> = ({
  ...
  // Load initial messages when opened / chat changes
  useEffect(() => {
    if (!isOpen || !chatId) return;
  ...
  // ✅ WS: connect + subscribe with loud logging
  useEffect(() => {
    if (!isOpen || !chatId) return;
    ...
    if (!token) {
      console.warn("[WS] No token -> cannot connect/subscribe");
      return;
    }
    ...
    (async () => {
      try {
        console.log("[WS] connecting…", { chatId });
        ...
        if (subRef.current) {
          console.log("[WS] unsubscribing previous subscription");
          subRef.current.unsubscribe();
          subRef.current = null;
        }

        console.log("[WS] subscribing…", `/topic/chats/${chatId}`);

        const sub = await chatSocket.subscribeToChat(token, chatId, (incoming) => {
          console.log("[WS] incoming ✅", incoming);

          const msg = incoming as MessageDTO;

          setMessages((prev) => {
            const exists = prev.some((m) => m.messageId === msg.messageId);
            if (exists) return prev;
            return normalize([...prev, msg]);
          });
        ...
        console.log("[WS] subscribed ✅", `/topic/chats/${chatId}`);
      } catch (e) {
        console.error("[WS] subscribe flow failed ❌", e);
        setWsConnected(false);
        setWsSubscribed(false);
      }
    })();
    ...
    return () => {
      ...
      if (subRef.current) {
        console.log("[WS] cleanup: unsubscribe", `/topic/chats/${chatId}`);
        subRef.current.unsubscribe();
        subRef.current = null;
      }
    };
  ...
  // ✅ Send via WS (broadcast will update messages via subscription)
  const send = async () => {
    const content = text.trim();
    if (!content || !chatId) return;
    if (!token) {
      console.warn("[WS] No token -> cannot send");
      return;
    }
    ...
    try {
      ...
      console.log("[WS] sending…", { chatId, content });
      await chatSocket.sendToChat(token, chatId, content);
      console.log("[WS] sent ✅");
    } catch (e) {
          ...
          <div className="chatwin__title">
            ...
            <div className="chatwin__titleText">
              ...
              <div style={{ fontSize: 12, opacity: 0.7, marginTop: 2 }}>
                ...
                {wsSubscribed ? "subscribed" : "not subscribed"}
              </div>
            </div>
          </div>
